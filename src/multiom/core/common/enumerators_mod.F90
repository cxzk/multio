...
IMPLICIT NONE
  ...
  ! Enumerators for ordering convention
  INTEGER(KIND=JPIB_K), PARAMETER :: ORDERING_CONVENTION_RING_E=1_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: ORDERING_CONVENTION_NESTED_E=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: N_ORDERING_CONVENTION_E=2_JPIB_K
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: STREAM_SFDD_E=1260_JPIB_K
  ...
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_IFS_NEMO_E=2_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_IFS_FESOM_E=3_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_AIFS_SINGLE_E=4_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_AIFS_ENS_E=5_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_AIFS_SINGLE_MSE_E=6_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_AIFS_ENS_CRPS_E=7_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: MODEL_AIFS_ENS_DIFF_E=8_JPIB_K
  INTEGER(KIND=JPIB_K), PARAMETER :: N_MODELS=9_JPIB_K
...
END INTERFACE
  ...
  PUBLIC :: ORDERING_CONVENTION_RING_E
  PUBLIC :: ORDERING_CONVENTION_NESTED_E
  PUBLIC :: N_ORDERING_CONVENTION_E
  ...
  ! Enumerators for models
  PUBLIC :: MODEL_IFS_E
  PUBLIC :: MODEL_IFS_NEMO_E
  PUBLIC :: MODEL_IFS_FESOM_E
 ...
 ! Integer binary operation
  ...
  PUBLIC :: IORDERING_CONVENTION2CORDERING_CONVENTION
  PUBLIC :: CORDERING_CONVENTION2IORDERING_CONVENTION
...
#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'IORDERING_CONVENTION2CORDERING_CONVENTION'
PP_THREAD_SAFE FUNCTION IORDERING_CONVENTION2CORDERING_CONVENTION( IORDERING_CONVENTION, CORDERING_CONVENTION, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  INTEGER(KIND=JPIB_K), INTENT(IN)    :: IORDERING_CONVENTION
  CHARACTER(LEN=16),    INTENT(OUT)   :: CORDERING_CONVENTION
  TYPE(HOOKS_T),        INTENT(INOUT) :: HOOKS
  ...
  !> Local error codes
  INTEGER(KIND=JPIB_K), PARAMETER :: ERRFLAG_UNKNOWN_ORDERING_CONVENTION=1_JPIB_K
  ...
  !> Initialization of the output variable
  CORDERING_CONVENTION = REPEAT(' ', 16)

  !> Select the prefix
  SELECT CASE ( IORDERING_CONVENTION )

  CASE ( ORDERING_CONVENTION_RING_E )
    CORDERING_CONVENTION = 'ring'
  CASE ( ORDERING_CONVENTION_NESTED_E )
    CORDERING_CONVENTION = 'nested'
  CASE DEFAULT
    PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNKNOWN_ORDERING_CONVENTION )
  END SELECT
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    ! Handle different errors
    SELECT CASE(ERRIDX)
    CASE (ERRFLAG_UNKNOWN_ORDERING_CONVENTION)
      TMPSTR = REPEAT(' ', 16)
      WRITE(TMPSTR,*) IORDERING_CONVENTION
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unknown HEALPix ordering convention: '//TRIM(ADJUSTL(TMPSTR)) )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION IORDERING_CONVENTION2CORDERING_CONVENTION
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE


#define PP_PROCEDURE_TYPE 'FUNCTION'
#define PP_PROCEDURE_NAME 'CORDERING_CONVENTION2IORDERING_CONVENTION'
PP_THREAD_SAFE FUNCTION CORDERING_CONVENTION2IORDERING_CONVENTION( CORDERING_CONVENTION, IORDERING_CONVENTION, HOOKS ) RESULT(RET)
...
IMPLICIT NONE
  ...
  !> Dummy arguments
  CHARACTER(LEN=*),     INTENT(IN)    :: CORDERING_CONVENTION
  INTEGER(KIND=JPIB_K), INTENT(OUT)   :: IORDERING_CONVENTION
  TYPE(HOOKS_T),        INTENT(INOUT) :: HOOKS
  ...
  !> Local variables
  CHARACTER(LEN=LEN_TRIM(CORDERING_CONVENTION)) :: LOC_CORDERING_CONVENTION
  ...
  !> Initialization of the output variable
  IORDERING_CONVENTION = UNDEF_PARAM_E

  !> Convert prefix to lowercase
  PP_TRYCALL(ERRFLAG_UNABLE_TO_CONVERT_LC) TOLOWER( CORDERING_CONVENTION, LOC_CORDERING_CONVENTION, HOOKS )

  !> Select the prefix
  SELECT CASE ( TRIM(ADJUSTL(LOC_CORDERING_CONVENTION)) )
  CASE ( 'ring' )
    IORDERING_CONVENTION = ORDERING_CONVENTION_RING_E
  CASE ( 'nested' )
    IORDERING_CONVENTION = ORDERING_CONVENTION_NESTED_E
  CASE DEFAULT
    PP_DEBUG_CRITICAL_THROW( ERRFLAG_UNKNOWN_ORDERING_CONVENTION )
  END SELECT
...
!$omp critical(ERROR_HANDLER)
  ...
  BLOCK
    ...
    CASE (ERRFLAG_UNABLE_TO_CONVERT_LC)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unable to convert to lowercase' )
    CASE (ERRFLAG_UNKNOWN_ORDERING_CONVENTION)
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unknown HEALPix ordering convention: '//TRIM(ADJUSTL(CORDERING_CONVENTION)) )
    CASE DEFAULT
      PP_DEBUG_PUSH_MSG_TO_FRAME( 'unhandled error' )
    END SELECT
...
END FUNCTION CORDERING_CONVENTION2IORDERING_CONVENTION
#undef PP_PROCEDURE_NAME
#undef PP_PROCEDURE_TYPE
...
IMPLICIT NONE
  ...
  CASE ( MODEL_IFS_E )
    CMODEL = 'IFS'
  CASE ( MODEL_IFS_NEMO_E )
    CMODEL = 'IFS-NEMO'
  CASE ( MODEL_IFS_FESOM_E )
    CMODEL = 'IFS-FESOM'
...
IMPLICIT NONE
  ...
  CASE ( 'ifs' )
    IMODEL = MODEL_IFS_E
  CASE ( 'ifs-nemo' )
    IMODEL = MODEL_IFS_NEMO_E
  CASE ( 'ifs-fesom' )
    IMODEL = MODEL_IFS_FESOM_E
  CASE ( 'aifs-single' )
